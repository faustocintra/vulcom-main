...
/*
    ...
    passado) para ter, pelo menos, 18 anops na data atual
*/
...
const Customer = z.object({
    ...
    house_number: z.string()
        ...
        .max(10, { message: 'O número do imóvel deve ter, no máximo, 10 caracteres.' }),

    complements: z.string()
        .trim()
        .max(20, { message: 'Complemento deve ter, no máximo, 20 caracteres.' })
        .nullish(),

    district: z.string()
        .trim()
        .min(1, { message: 'Bairro deve ter, no mínimo, 1 caractere.' })
        .max(25, { message: 'Bairro deve ter, no máximo, 25 caracteres.' }),

    municipality: z.string()
        .trim()
        .min(1, { message: 'Município deve ter, no mínimo, 1 caractere.' })
        .max(40, { message: 'Município deve ter, no máximo, 40 caracteres.' }),
